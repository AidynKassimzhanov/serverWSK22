    ...
    try {
      ...
      for (let i = 1; i <= 20; i++) {
          gamesData.push({
              ...
              PlatformUserId: dev1.id // Предполагая, что вы хотите связать игры с dev1
          });
      }

      // Используем цикл для создания объектов
      const gamesPromises = gamesData.map(game => {
          return Game.create(game);
      });

      // После создания всех объектов используем bulkCreate
